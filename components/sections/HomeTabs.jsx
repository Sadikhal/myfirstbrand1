...
const HomeTabs = () => {
  return (
    <div className='xl:w-[80%] flex w-full'> 
        ...
        Teamswork.map((item) => ( 
          <div
          key={item.id} className='text-left w-full'>
        ...
        <AccordionTrigger className='text-left font-teko leading-[32px] font-normal text-[20px] uppercase  cursor-pointer tracking-[1px] w-full data-[state=open]:text-[24px]'>{item.title}</AccordionTrigger>
        <AccordionContent className="w-full ">
        
          <img src={item.img} className='object-cover rounded-lg grayscale-[89%] hover:grayscale-0' alt="banners" />
        </AccordionContent>
        <AccordionContent className="font-rubik text-[16px] font-normal leading-[2.125em] text-left w-full">
          {item.desc}
        </AccordionContent>
          {
            item.id === 106 &&
